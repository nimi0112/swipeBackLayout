package com.tutor.feel_better.swipetry.library;
...
import com.tutor.feel_better.swipetry.CommonActivity;
import com.tutor.feel_better.swipetry.R;
...
public class SwipeBackLayout extends ViewGroup {
    ...
    private int mDirectionMode = FROM_BOTTOM;
    ...
    public SwipeBackLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mDragHelper = ViewDragHelper.create(this, 25f, new DragHelperCallback());
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        //Log.e("isTargetView",String.valueOf(isTargetView(ev)));
        ...
        if (isTargetView(ev) && mDragHelper.shouldInterceptTouchEvent(ev)) {
            return true;
        } else {
            return super.onInterceptTouchEvent(ev);
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (isTargetView(event) || isMoving() ){
...
//return true;
    ...
    private boolean isTargetView(MotionEvent event) {

        Rect rect=new Rect();
        swipe_button.getHitRect(rect);
//        Log.e("top",String.valueOf(rect.top));
  //      Log.e("bottom",String.valueOf(rect.bottom));

        int upperLimit = rect.top+changedTop;
        int lowerLimit = rect.bottom+changedTop;
        int limit=lowerLimit-upperLimit;
        upperLimit=upperLimit-limit;
        lowerLimit=lowerLimit+limit;


        int y = (int) event.getRawY();
       // Log.e("y value",String.valueOf((int) event.getRawY()));
     //   Log.e("x value",String.valueOf((int) event.getRawX()));
        return (y < lowerLimit && y > upperLimit);
    }

    public void setButton(int arr[]){
        //Log.e("arre", Arrays.toString(arr)+"inside swipe");
        this.btnlca=arr;
    }
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            changedTop=top;
            //Log.e("top is ",String.valueOf(top));
            //Log.e("dx is ",String.valueOf(dx));
            //Log.e("dy is ",String.valueOf(dy));
